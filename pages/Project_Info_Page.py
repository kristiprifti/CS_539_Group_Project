...
The goal and primary function of the team’s tool is to help users make well informed decisions regarding which fast-food
 ...
 trends such as time of year, geographic location, and proximity to competitors.  The goal of this application is to act
...
major and persisting issues for fast food restaurants. The team hopes to aid in identifying these issues, whether that 
be simply identifying whether to stop at a particular McDonald’s, or to keep driving. We hope to also inform users of 
...
the reviews themselves. Our tool’s goal would be to highlight the important features of a successful fast-food chain 
...
To build this tool, the team would need to collect store reviews across multiple locations, latitude and longitude 
...
of this tool, so this will be the main challenge.  
...
As mentioned previously, the team will need to collect fast-food chain store reviews, latitude and longitude positions 
of competing fast-food chains, and census data to get full understanding on what makes an individual establishment 
successful. For this tool, the fast-food chain we will be selecting is McDonald’s given its popularity across the 
...
Regardless of the algorithm, we plan to preprocess the text using NLP Python libraries like spaCy or NLTK, which will 
deal with the necessary preprocessing. Sentiment analysis can give us more insight into the customer's attitude going 
beyond their numerical star rating. We can start with models like Naive Bayes, Logistic Regression, or SVM, then move 
to more complex models for higher accuracy, like CNN with Word2Vec and RNNs for sentiment analysis. With the insight 
we gained from sentiment analysis, we can use NLP topic modeling to extract common topics and themes in the reviews. 
Topic modeling combined with sentiment analysis will narrow down the specific aspects of McDonald's that are positive 
or negative. Since the dataset includes location data, we could use sentiment analysis from the reviews to build a 
...
Our approach would be to start with the simpler models, evaluate their performance, and then move towards more complex
